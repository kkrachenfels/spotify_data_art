...
const loginBtn = el(
  "button",
  { class: "compact-button auth-button", onclick: () => (window.location = "/login") },
  "Login with Spotify"
);
const logoutBtn = el(
  "button",
  { class: "compact-button auth-button", onclick: () => (window.location = "/logout") },
  "Logout"
);
...
const controlColumn = el("div", { class: "control-column" }, filterSection, rangeStatus);
...
const fruitCaption = el("p", { class: "fruit-caption" }, "Load top tracks to start the fruit preview.");
const fruitPanel = el(
  "div",
  { class: "fruit-panel" },
...
const visualColumn = el("div", { class: "visual-column" }, fruitPanel, vinylPanel);
const contentLayout = el("div", { class: "content-layout" }, controlColumn, visualColumn);
let fruitQueue = [];
...
function resetFruitSequence(tracks) {
  ...
  if (!fruitQueue.length) {
    ...
    if (fruitCtx) fruitCtx.clearRect(0, 0, FRUIT_CANVAS_WIDTH, FRUIT_CANVAS_HEIGHT);
    return;
  }
  spawnNextFruit();
...
function spawnNextFruit() {
  ...
  fruit.setPulseStyle(0.92, 1.08, (fruitSpawnIndex / Math.max(fruitQueue.length, 1)) * Math.PI);
  fruitObjects.splice(0, fruitObjects.length, fruit);
  fruitCaption.textContent = `${track.rank ? `#${track.rank} ` : ""}${track.name}`;
  fruitSpawnIndex += 1;
  if (fruitSpawnIndex >= fruitQueue.length) {
    stopFruitInterval();
  }
